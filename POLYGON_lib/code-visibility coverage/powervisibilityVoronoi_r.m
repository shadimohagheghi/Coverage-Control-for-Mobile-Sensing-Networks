function [visVor]=powervisibilityVoronoi_r(p,pol,r)
...
for i=1:length(visIntersection)
    if ~isempty(visIntersection(i).int)
        inters=visIntersection(i).int;%tomh twn vispol
        vcell=power_voronoi_limited_2centers(p(visIntersection(i).ind,:),pol,r,p);
