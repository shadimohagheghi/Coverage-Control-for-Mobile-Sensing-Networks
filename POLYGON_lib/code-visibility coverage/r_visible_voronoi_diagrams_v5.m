function [vor,r_lim]=r_visible_voronoi_diagrams_v5(x,y,x1,y1,r,N)
...
%            for t1=1:length(fx2)
%                [d,x_poly,y_poly] = p_poly_dist(fx2(t1), fy2(t1), x_pol, y_pol) ;
%                 if abs(d)<10^(-8)
%                     d=0;%on the boundary=>(inside)           
%                 end
%                 if d<=0 
%                     fx7(id)=fx2(t1);fy7(id)=fy2(t1);...%tmhma tou edge pou einai orato ston j
%                     id=id+1;    
%                 end   
%            end
            
           %[fx7,fy7]=polyxpoly(fx2,fy2,x_pol,y_pol);...%tmhma tou edge pou einai orato ston j
            %ALLAGH ME INPOLYGON TOU FX2 STO X_POL!!!!!!!   
...
%            end
           ...
           if length(fx6)<=0 %einai eite plhrws orato eite plhrws aorato
             ...
             %   in=inpolygon(fx2(1),fy2(1),x_pol2,y_pol2);%ALLAGH  ..UPARXOUN ARITHMITIKA SFALMATA  p_poly_dist
                [d1,x_poly,y_poly] = p_poly_dist(fx5(1), fy5(1), x_pol2, y_pol2) ;
                [d2,x_poly,y_poly] = p_poly_dist(fx5(2), fy5(2), x_pol2, y_pol2) ;
                if abs(d1)<10^(-8)
                    d1=0;%on the boundary=>(inside) %d<=0 inside(visible)          
                end
                if abs(d2)<10^(-8)
                    d2=0;%on the boundary=>(inside) %d<=0 inside(visible)          
                end
                %an in=0 den einai visible
                if d1>0 && d2>0 && ~isempty(fx7) %an den einai visible (d<0)apo ton del(j2) alla einai visible apo ton j
                        E=non_visible_line(fx5,fy5,xv,yv);
                        [xv1,yv1]=insert_E_in_vornoi_cell(xv,yv,fx2,fy2,E); %PROSOXH!!!!!!!!!!!
                        xv=xv1;yv=yv1;
%                         plot(xv,yv,'red')  
                %else:do nothing    
                end
            ...
            if  ~isempty(fx6) % sthn antitheth periptwsh den einai katholou orato
                %[flag,index]=choose_subcase_for_case2(fx6,fy6,fx2,fy2);
                 ...
                 if  length(fx6)==2 %flag==1    %2.1)  oi akres tou tmhmatos autou den tautizontai me tis akres tou euthigrammou tmimatos tou koinou edge (fx2,fy2)
                  ...
                  %-----gia thn prwth korufh tou apo koinou oratou tmhmatos-----
                    ...
                    % vriskw to antistoixo reflex vertex...dld perpatw panw sto vis_pol  tou del geitona mou
                     ...
                     D1=define_BD_segment_V2(xv,yv,fx2,fy2,B1,xp2,yp2);
                  ...
                  %------gia thn deuterh korufh tou apo koinou oratou tmhmatos-----
                   ...
                   xedge2=[xcand2 fx6(2)]; yedge2=[ycand2 fy6(2)];...
                    ...
                    D2=define_BD_segment_V2(xv,yv,fx2,fy2,B2,xp2,yp2);
                 ...
                 elseif length(fx6)==1%flag==2 %2.2) estw mia akrh autou tou tmhmatos tautizetai me mia apo tis akres tou koinou edge (fx2,fy2)
                     ...
                     %proekteinw thn BD pros to B    
                     E1=extend_BD_v2(B1,D1,xedge1,yedge1,x1,y1);...
                     [xv1,yv1]=insert_points_in_voronoi_cell_V2(xv,yv,Ak1,B1,E1,D1,fx6(1),fy6(1),xp2,yp2);
                     xv=xv1;yv=yv1; 
                     plot(xv,yv,'r')  
